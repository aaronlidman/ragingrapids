...
function getPageByTitle(title, direction) {
    return getPage(urlFromTitle(title, direction), title);
}

function urlFromTitle(title, direction) {
    title = normalizeTitle(title);
    const apiUrl = url[direction].replace('{title}', title);
    return apiUrl;
}
...
function parseContinue(pageResults, url) {
    if (pageResults.continue) {
        if (pageResults.continue.plcontinue) {
            Promise.resolve(url + ('&plcontinue=' + theContinue));
        } else if (pageResults.continue.lhcontinue) {
            Promise.resolve(url + ('&lhcontinue=' + theContinue));
        }        
    } else {
...
function parseLinks(pageResults) {
    ...
    for (const page in pages) {
        if (!linkType) linkType = pages[page].links ? 'links' : 'linkshere';
