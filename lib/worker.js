...
function updateJobStatus(results, message) {
    return new Promise((resolve, reject) => {
        if (results.foundDestination) {
            log.info('found destination!', results.newPath.concat(message.destination).reverse());
...
function findConnectors(status, links, message) {
    ...
    return new Promise((resolve, reject) => {
        ...
        client.hmget(message.source, links, (err, results) => {
            if (err) return reject(err);
            ...
            if (results[0]) console.log('hey', results[0]);

            if (results.length) return resolve({
                foundDestination: true,
                newPath: message.path.concat([results[0]])
            });
...
function updateGraph(links, message) {
    ...
    return new Promise((resolve, reject) => {
        ...
        links.forEach(link => {
            ...
            if (backward) path.reverse();
            setObj[link] = JSON.stringify(path);
        });

        client.hmset(message.destination, setObj, (err, results) => {
            if (err) {
