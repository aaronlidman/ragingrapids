...
function processMessage(response) {
    ...
    jobIsActive(message.jobId)
        ...
        .then(result => updateJobStatus(response.id, result))
...
function jobIsActive(jobId) {
    ...
    return new Promise((resolve, reject) => {
        client.hget('wikiracer:job:' + jobId, 'active', (err, response) => {
...
function updateJobStatus(id, foundInDestination) {
    // todo: update the status with any changes
    // what other interesting things could we stuff in there?

    // todo: update job status in redis
        // make sure not to override if it's already been found
        // just update the flag? idk we need the final destination somehow
    if (foundInDestination) log.info('found destination');

    return Promise.resolve({
        found: foundInDestination
    });
}

function queueLinks(status, links, message) {
    // if the destination was found, don't queue anything just resolve
    if (status.found) return Promise.resolve();
    ...
    // todo: do the actual queuing, in bulk somehow?

    console.log('queuing', messages.map(JSON.stringify).join('\n'));
